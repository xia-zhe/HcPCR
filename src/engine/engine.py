...
def train_one_epoch(model: torch.nn.Module,
    ...
    # pretrained_weight = "/C/numcc/pretrained/udf-ep99.pth"
    # assert os.path.exists(pretrained_weight),"file {} does not exist.".format(pretrained_weight)
    # model.load_state_dict(torch.load(pretrained_weight, map_location=device),strict=False)
    ...
    for data_iter_step, samples in enumerate(data_loader):
        ...
        # out:[B, 550, 769]  query_xyz2:[B, 3, 550] pos_data1:[B, 550, k1] 以随机某个点为中心找最近k1个为大整体
        # pos_data1:[B, 550, k2] n_points=k-1, 第二次get_children_np是在
        query_xyz2 = query_xyz.permute(0, 2, 1)
        # mar_par, pos_data1, query_rgb_gt, labels, n_points = get_children_np(query_xyz2, query_rgb_gt, labels, kmin = 550, kmax = 550)       #800-550
        # mar, pos_data2, _, _, _ = get_children_np(pos_data1, starting = n_points, kmin = 400, kmax = 550)  #50-600
        # mar, pos_data2, _, _, _ = get_children_np(query_xyz2, kmin = 400, kmax = 550)  #50-600

        out, centers_xyz, t_loss, h_loss = model(
            seen_images=seen_images,
            seen_xyz=seen_xyz,
            query_xyz=query_xyz,
            valid_seen_xyz=valid_seen_xyz,
        ...
        # out2, centers_xyz = model(
        #     seen_images=seen_images,
        #     seen_xyz=seen_xyz,
        #     query_xyz=pos_data2.permute(0, 2, 1),
        #     valid_seen_xyz=valid_seen_xyz,
        #     seen_xyz_hr=seen_xyz_hr,
        #     valid_seen_xyz_hr=valid_seen_xyz_hr
        # )

        # pn, posn, pd, nd, t_loss, h_loss = hype_triplet_losses(out, out2, hier_margin=mar, contr_margin=4, ball_dim = 256)


        # center loss
        # loss_centers = loss_fns['centers'](centers_xyz.float(), gt_fps_xyz, norm=args.cd_norm)[0]*0.03
        # loss_centers1 = chamfer_distance(centers_xyz.float(), gt_fps_xyz, norm=args.cd_norm)[0]*0.03
        loss_centers = loss_fns['centers'](centers_xyz.float(), gt_fps_xyz)[0]*0.03
        ...
        # loss = loss_centers + loss_geo + loss_rgb + t_loss*0.1 + h_loss*0.1
        loss = loss_centers + loss_geo + loss_rgb + t_loss*0.01
        ...
        if (data_iter_step % args.print_every)== 0 and (data_iter_step > 0):
            print('[Epoch %02d] it=%03d/%03d, loss=%.4f, loss_geo=%.4f, loss_rgb=%.4f, loss_center=%.4f, t_loss=%.4f, h_loss=%.4f' % (epoch, data_iter_step, len(data_loader), 
                                                                                                            loss_value, loss_geo, loss_rgb, loss_centers, t_loss, h_loss))

            logging_string = '[Epoch %02d] it=%03d/%03d, loss=%.4f, loss_geo=%.4f, loss_rgb=%.4f, loss_center=%.4f, t_loss=%.4f, h_loss=%.4f' % (epoch, data_iter_step, len(data_loader), 
                                                                                                                loss_value, loss_geo, loss_rgb, loss_centers, t_loss, h_loss)
...
def prepare_data_udf(samples, device, is_train, args, is_viz=False):
    ...
    # Gt
    gt_xyz, gt_rgb = samples[1][0].to(device).reshape(B, -1, 3), samples[1][1].to(device).reshape(B, -1, 3)     # [16, 20000, 3]
    gt_xyz = shrink_points_beyond_threshold(gt_xyz, args.shrink_threshold)

    if is_train:  # 数据增强, 旋转放缩
    ...
    gt_fps_xyz, _ = sample_farthest_points(gt_xyz, K=args.n_groups) # B, M, 3   # [16, 200, 3] 
