...
class FeatureAggregator(nn.Module):
    ...
    def sample_grid_features(self, xyz_q, seen_xyz, valid_seen_xyz, grid_feat):
        ...
        with torch.no_grad():
            dists = square_distance(xyz_q, seen_xyz)  # val [1, 12000, 3] [1, 12544, 3]  now [1, 8797, 3] [16, 12544, 3] nomal [16, 550, 3] [16, 12544, 3]
    ...
    def forward(self, xyz_q, seen_xyz, valid_seen_xyz, fea, up_grid_fea, custom_centers=None):
        ...
        lat_rep = fea['global_feats']           # [16, 512]

        if custom_centers is not None:
            xyz = custom_centers
        else:
            xyz = fea['anchors_xyz']            # [16, 200, 3]

        feats = fea['anchors_feats']            # [16, 200, 512]
        ...
        if self.args.no_fine == 1:
            ...
            sampled_grid_feat, nn_seen_loc = self.sample_grid_features(xyz_q, seen_xyz.clone(), valid_seen_xyz, grid_feat) # sampled_grid_feat [16, 550, 4, 512]  nn_seen_loc [16, 550, 4, 3]
            lat_rep = self.ct1(xyz_q, lat_rep, xyz, feats, sampled_grid_feat, nn_seen_loc)              # [16, 550, 512]
