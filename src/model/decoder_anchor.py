...
class DecoderPredictCenters(nn.Module):
    ...
    def forward(self, x): 
        ...
        # Initialize centers
        input = torch.ones(B, self.M, device=self.device) * torch.linspace(0, self.M-1, steps=self.M, device=self.device)   # [16, 200]
        init_embedding = self.init_embedding(input.int())                                                                   # B, M, d [16, 200, 512]

        # embed tokens
        y = self.decoder_embed(x)                           # [16, 197, 512]

        init_global = y[:,0,:].unsqueeze(1)                 # [16, 1, 512]
        init_embedding = init_embedding + init_global       # [16, 200, 512]

        # Pos embed input vit tokens
        y = y + self.decoder_pos_embed                      # [16, 197, 512]

        # 3D pos embed
        y = torch.cat([y, init_embedding], dim=1)           # [16, 397, 512]

        # apply Transformer blocks
        for blk in self.decoder_blocks:
            y = blk(y)

        y = self.decoder_norm(y)                            # [16, 397, 512]
        center_fea = self.decoder_pred(y[:,-self.M:, :])

        fea['enc_feats'] = x                                # [16, 197, 1536]
        fea['global_feats'] = y[:,0,:]                      # [16, 512]
        fea['anchors_xyz'] = center_fea[:, :, :3]           # [16, 200, 3]
        fea['anchors_feats'] = center_fea[:,:,3:]           # [16, 200, 512]
