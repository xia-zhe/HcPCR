...
class MCCEncoder(nn.Module):
    ...
    def forward(self, seen_images, seen_xyz, valid_seen_xyz, up_grid_bypass=None):
        ...
        y = self.xyz_pos_embed(seen_xyz, valid_seen_xyz)            # y [16, 197, 768]
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)                                            # x [16, 197, 768]

        # combine encodings
        xy = torch.cat([x, y], dim=2) # B x (PatchPatch) x 2dim     # xy [16, 197, 1536]

        fine_fea = {}
        fine_fea['rgb'] = fine_fea_rgb                              # fine_fea_rgb [16, 3, 112, 112]
