    ...
    def __getitem__(self, index):
        for retry in range(1000):
            try:
                ...
                assert gap >= 1
                category, sequence_name = self.all_example_names[(index * gap) % len(self.all_example_names)] # '110_13070_25581'
                ...
                seen_rgb = frame_data.image_rgb.clone().detach()    # # [1, 3, 800, 800]
                ...
                seen_xyz = co3d_utils.get_rgbd_points(
                    ...
                    frame_data.camera,
                    frame_data.depth_map, #torch.Size([1, 1, 800, 800])
                    mask,
                )      # [112, 112, 3]
                ...
                full_point_cloud = co3d_utils._load_pointcloud(f'{self.args.co3d_path}/{category}/{sequence_name}/pointcloud.ply', max_points=20000)    # [20000]
                full_point_cloud = pad_point_cloud(full_point_cloud, 20000)
                break
            except Exception as e:
                print(category, sequence_name, 'sampling failed', retry, e)

        seen_rgb = seen_rgb.squeeze(0)  # [3, 800, 800]
        full_rgb = full_point_cloud._features_list[0]   # [20000, 3]
